...
Bitcoin.ECKey = (function () {
	...
	var ECKey = function (input) {
		if (!input) {
			...
			try {
				if (ECKey.isWalletImportFormat(input)) {
					...
					bytes = __Crypto__.SHA256(input, { asBytes: true });
				} else if (ECKey.isHexFormat(input)) {
					bytes = __Crypto__.util.hexToBytes(input);
				} else if (ECKey.isBase64Format(input)) {
					bytes = __Crypto__.util.base64ToBytes(input);
				}
			} catch (exc1) {
				this.setError(exc1);
			}
		...
		try {
			...
			var rangeLimitBytes = __Crypto__.util.hexToBytes(hexKeyRangeLimit);
	...
	ECKey.prototype.getPubKeyHex = function () {
		if (this.compressed) {
			...
			return (this.pubKeyHexComp = __Crypto__.util.bytesToHex(this.getPub()).toString().toUpperCase());
		} else {
			if (this.pubKeyHexUncomp) return this.pubKeyHexUncomp;
			return (this.pubKeyHexUncomp = __Crypto__.util.bytesToHex(this.getPub()).toString().toUpperCase());
		}
	};
	...
	ECKey.prototype.setPub = function (pub) {
		...
		if (Bitcoin.Util.isArray(pub)) {
			pub = __Crypto__.util.bytesToHex(pub).toString().toUpperCase();
		}
	...
	ECKey.prototype.getBitcoinWalletImportFormat = function () {
		...
		var checksum = __Crypto__.SHA256(__Crypto__.SHA256(bytes, { asBytes: true }), { asBytes: true });
	...
	ECKey.prototype.getBitcoinHexFormat = function () {
		return __Crypto__.util.bytesToHex(this.getBitcoinPrivateKeyByteArray()).toString().toUpperCase();
	};

	// Private Key Base64 Format
	ECKey.prototype.getBitcoinBase64Format = function () {
		return __Crypto__.util.bytesToBase64(this.getBitcoinPrivateKeyByteArray());
	};
	...
	ECKey.decodeWalletImportFormat = function (privStr) {
		...
		var checksum = __Crypto__.SHA256(__Crypto__.SHA256(hash, { asBytes: true }), { asBytes: true });
		if (
			checksum[0] != bytes[33] ||
			checksum[1] != bytes[34] ||
			checksum[2] != bytes[35] ||
	...
	ECKey.isMiniFormat = function (key) {
		...
		var testBytes = __Crypto__.SHA256(key + '?', { asBytes: true });
