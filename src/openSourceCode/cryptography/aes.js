...
(function () {
	...
	globalThis.__Crypto__.AES = {
		encrypt(message, password, options) {
			options = options || {};

			// Determine mode
			var mode = options.mode || new globalThis.__Crypto__.mode.OFB();

			// Allow mode to override options
			if (mode.fixOptions) mode.fixOptions(options);

			// Convert to bytes if message is a string
			var m = typeof message === 'string' ? UTF8.stringToBytes(message) : message,
				// Generate random IV
				iv = options.iv || util.randomBytes(this._blocksize * 4),
				// Generate key
				k =
					typeof password === 'string'
						? // Derive key from pass-phrase
						  globalThis.__Crypto__.PBKDF2(password, iv, 32, { asBytes: true })
						: // else, assume byte array representing cryptographic key
						  password;
			this._init(k);
			mode.encrypt(globalThis.__Crypto__.AES, m, iv);
			// Return ciphertext
			m = options.iv ? m : iv.concat(m);
			return options && options.asBytes ? m : util.bytesToBase64(m);
		},
		decrypt(ciphertext, password, options) {
			options = options || {};
			...
			var c = typeof ciphertext === 'string' ? util.base64ToBytes(ciphertext) : ciphertext,
				// Separate IV and message
				iv = options.iv || c.splice(0, this._blocksize * 4),
				// Generate key
				k =
					typeof password === 'string'
						? // Derive key from pass-phrase
			...
			// Decrypt
			this._init(k);
			mode.decrypt(globalThis.__Crypto__.AES, c, iv);
		...
		/**
		 * @description generate key schedule
		 */
		_keyexpansion(k) {
			keyschedule = [];
